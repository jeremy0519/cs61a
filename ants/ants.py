...
class Ant(Insect):
    ...
    def double(self):
        ...
        if not self.was_doubled:
            ...
            self.damage *= 2
        if isinstance(self, ContainerAnt) and self.ant_contained is not None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        offset = 0
        while (
            checking_place and not checking_place.is_hive and offset <= self.upper_bound
        ):
            if offset >= self.lower_bound and checking_place.bees:
                return random_bee(checking_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        "*** YOUR CODE HERE ***"
        if self.place is not None:
            # Damage Bees at its Place
            for bee in self.place.bees[:]:
                bee.reduce_health(
                    amount + self.damage if self.health - amount <= 0 else amount
                )
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate: GameState):
        if self.cooldown:  # Still chewing
            self.cooldown -= 1
        else:
            if self.place is not None:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other: Ant) -> bool:
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.ant_contained == None and other.is_container == False
        # END Problem 8a
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self) -> dict[Insect, int]:
        ...
        checking_place = self.place
        distance = 0
        while checking_place:
            # Ants:
            ans[checking_place.ant] = distance
            # Bee:
            for bee in checking_place.bees:
                ans[bee] = distance
            checking_place = checking_place.entrance
            distance += 1
        if self in ans:
    ...
    def calculate_damage(self, distance: int) -> float:
        # BEGIN Problem EC 4
        laser_damage = self.base_damage - 0.0625 * self.insects_shot
        ans = laser_damage - 0.25 * distance
...
class Bee(Insect):
    ...
    def blocked(self) -> bool:
        ...
        return (
            ...
            and self.place.ant is not None
            and self.place.ant.blocks_path
        )
        # END Problem EC 3

    def action(self, gamestate: GameState):
        """更新后的action，加入slow和scare效果的处理"""

        if self.scare_turns:
            if self.slow_turns:  # 合并slow和scare效果
                if gamestate.time % 2 == 0:
                    self.action_move(gamestate, "backward")
                    self.scare_turns -= 1
                self.slow_turns -= 1
            else:  # 有scare无slow效果
                self.action_move(gamestate, "backward")
            ...
            if self.slow_turns:  # 有slow无scare效果
                if gamestate.time % 2 == 0:
                    self.action_move(gamestate, "forward")
                self.slow_turns -= 1
            else:  # 没有任何效果
                self.action_move(gamestate, "forward")  # 运行初始action

    def action_move(
        self, gamestate: GameState, direction: Literal["forward", "backward"]
    ):
        ...
        if self.place:
            destination = (
                self.place.entrance if direction == "backward" else self.place.exit
            )
    ...
    def scare(self, length: int):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if self.was_scared and self.scare_turns == 0:
            return  # 不会受到scare效果
        # 初始化scare参数：
...
class GameState:
    ...
    def __init__(
        self, beehive: Hive, ant_types: list, create_places, dimensions, food: int = 2
    ):
