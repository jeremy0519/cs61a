...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ans = [paragraph for paragraph in paragraphs if select(paragraph)]
...
def autocorrect(
    ...
    for i in range(len(word_list)):
        diff = min(diff, (diff_function(typed_word, word_list[i], limit), i))
    assert diff[1] != None
    return typed_word if diff[0] > limit else word_list[diff[1]]
    # END PROBLEM 5
...
def minimum_mewtations(typed: str, source: str, limit: int) -> int:
    ...
    remove = 1 + minimum_mewtations(typed[1:], source, limit - 1)  # 删掉source[0]
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    # BEGIN PROBLEM 9
    times = [
        [tpp[player][i + 1] - tpp[player][i] for i in range(len(tpp[player]) - 1)]
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    ans: list[list[str]] = [[]] * len(times)
    for word in word_indices:
        min_time = float("inf")
        for player in player_indices:
            player_time = get_time(times, player, word)
            if player_time < min_time:
                min_player = player
                min_time = player_time
        ans[min_player] = ans[min_player] + [words[word]]
    return ans
    # END PROBLEM 10
