...
def do_define_form(expressions: Link, env: Frame, tail=False):
    ...
    if scheme_symbolp(signature):
        ...
        "*** YOUR CODE HERE ***"
        env.define(
            signature.first, do_lambda_form(Link(signature.rest, expressions.rest), env)
        )
...
def do_lambda_form(expressions, env, tail=False):
    ...
    )  # 这里还没有evaluate，到scheme_apply那里才evaluate
    # END PROBLEM 7
...
def do_and_form(expressions: Link, env, tail=False):
    ...
    while expressions.rest != nil:
        if is_scheme_false(scheme_eval(expressions.first, env)):
            return False  # 遇到falsy提前结束
        expressions = expressions.rest
    return scheme_eval(expressions.first, env, tail)  # 返回最后一个
    # END PROBLEM 12
...
def do_or_form(expressions, env, tail=False):
    ...
    if expressions == nil:
        return False
    while expressions.rest != nil:
        now = scheme_eval(expressions.first, env)
        if is_scheme_true(now):  # 遇到truthy提前结束
            return now
        expressions = expressions.rest
    return scheme_eval(expressions.first, env, tail)  # 返回最后一个
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        names = Link(current.first, names)
        vals = Link(scheme_eval(current.rest.first, env), vals)
