...
def scheme_eval(expr, env: Frame, tail=False):
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:  # special forms
        ...
        "*** YOUR CODE HERE ***"
        return scheme_apply(
            scheme_eval(first, env),  # procedure instance
            map_link(lambda x: scheme_eval(x, env), rest),  # arguments
            env,
        )
        # END PROBLEM 3


def scheme_apply(procedure: Procedure, args: Link, env: Frame):
    """Apply Scheme PROCEDURE to argument values ARGS (a Scheme list) in
    Frame ENV, the current environment."""
    validate_procedure(procedure)
    if not isinstance(env, Frame):
        assert False, "Not a Frame: {}".format(env)
    if isinstance(procedure, BuiltinProcedure):  # Apply builtin procedure
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        arguments = []
        while args:
