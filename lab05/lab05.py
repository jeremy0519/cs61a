...
from typing import (
    Iterator,
)  # "t: Iterator[int]" means t is an iterator that yields integers
...
def count_occurrences(t: Iterator[int], n: int, x: int) -> int:
    ...
    """
    "*** YOUR CODE HERE ***"
    return sum(next(t) == x for _ in range(n))
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return tree(label(t), [tree(num) for num in leaves])
...
def balanced(t):
    ...
    if is_leaf(t):
        return True
    return len(set(sum_tree(branch) for branch in branches(t))) <= 1 and all(
        balanced(branch) for branch in branches(t)
    )
...
def partial_reverse(s: list[int], start: int) -> None:
    ...
    for i in range(n // 2 + n % 2):
        s[i + start], s[-1 - i] = s[-1 - i], s[i + start]
