...
def calc_eval(exp):
    ...
    if isinstance(exp, Link):
        ...
        if operator == "and":  # and expressions
            ...
            return calc_apply(
                (OPERATORS[operator] if operator in OPERATORS else bindings[operator]),
                operands,
...
def floor_div(args: Link):
    ...
    """
    "*** YOUR CODE HERE ***"
    dividend, divisor = calc_eval(args.first), calc_eval(args.rest.first)
    if type(dividend) != int or type(divisor) != int:
        raise TypeError("floor_div must have int type args")
    return (
        dividend // divisor
        if args.rest.rest == nil
        else floor_div(Link(dividend // divisor, args.rest.rest))
    )
...
def eval_define(expressions: Link):
    """
...
OPERATORS = {
    "//": floor_div,
    "+": addition,  # type: ignore
    "-": subtraction,  # type: ignore
    "*": multiplication,  # type: ignore
    "/": division,  # type: ignore
}
